...
export function JudgesTab({ competitionId }: { competitionId: string }) {
  ...
  const createTokens = useMutation({
    mutationFn: () => judgeTokensApi.create(competitionId, parseInt(count) || 1),
    onSuccess: () => {
  ...
  return (
    <div className="flex flex-col gap-6">
          ...
          <TableBody>
            ...
            {tokens?.map((tok) => (
              <TableRow key={tok.id}>
                ...
                <TableCell><code className="rounded bg-[var(--surface-secondary)] px-2 py-0.5 text-xs">{tok.token.slice(0, 16)}…</code></TableCell>
                <TableCell><PinCell pin={(tok as JudgeTokenDto & { pin?: string }).pin} /></TableCell>
                ...
                <TableCell>
                  <div className="flex items-center gap-1">
                    ...
                    <Button variant="ghost" size="icon-sm" onClick={() => { navigator.clipboard.writeText(`${judgeBaseUrl}/${tok.token}`); toast({ title: t("judges.linkCopied") } as Parameters<typeof toast>[0]); }}><Copy className="h-3.5 w-3.5" /></Button>
            ...
            {!isLoading && !tokens?.length && (
              <TableRow><TableCell colSpan={5} className="py-12 text-center text-[var(--text-secondary)]">{t("judges.noTokens")}</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </Card>
          ...
          <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
            {activeTokens.map((tok) => (
              <button key={tok.id} onClick={() => setQrToken(tok)} className="flex flex-col items-center gap-2 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-4 text-center transition-all hover:border-[var(--accent)]/40 hover:shadow-md">
                ...
                <QRCanvas url={`${judgeBaseUrl}/${tok.token}`} size={100} />
      ...
      {/* Activity log */}
      <Card>
        <CardHeader><CardTitle className="text-sm">{t("judges.activityLog")}</CardTitle></CardHeader>
        <Table>
          <TableHeader>
            <TableRow>
          ...
          <TableBody>
            {MOCK_ACTIVITY.sort((a, b) => new Date(b.at).getTime() - new Date(a.at).getTime()).map((entry, i) => (
              <TableRow key={i}>
                <TableCell className="font-medium">{t("judges.judgeNumber", { number: String(entry.judgeNumber) })}</TableCell>
                <TableCell>
                  <Badge variant={entry.event === "Scores submitted" ? "success" : entry.event === "Connected" ? "default" : "warning"}>{entry.event}</Badge>
                </TableCell>
                <TableCell className="text-xs text-[var(--text-secondary)]">{formatTime(entry.at)}</TableCell>
                <TableCell className="text-xs text-[var(--text-tertiary)]">{entry.note || "—"}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
